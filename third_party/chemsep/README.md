# third_party/chemsep — ChemSep Database (Artistic License 2.0)
...
| `LICENSE-Artistic-2.0.txt` | canonical licence text (ships with the download) | provenance hygiene |

Get them from the free ChemSep distribution (https://www.chemsep.org/) or a
DWSIM install.  **Do not** commit large raw databases unless intended; the
importer reads them from here and writes the curated subset to `data/proposed/`.

## Workflow (curate → review → promote)
```
1. drop chemsep1.xml (+ *.ipd) into this folder
2. python3 bin/curate/chemsep_to_choupo.py
