...
| **S5** | Witnesses: the Marcilla LLS tie-triangle (aqueous + organic + solid NaCl at once — the flagship; anchors staged in [`solid-migration-witness-data.md`](solid-migration-witness-data.md)); the ice + dihydrate eutectic once the dihydrate is curated (Vítor's, non-blocking per R3) | **S5a SHIPPED 2026-08-10** — anchor contact made, machinery complete, the tie-triangle itself BLOCKED on a named model/data gap.  `marcilla01_lls_tie_triangle` charges the paper's own Table 10A row 4 mixture: the brine + solid-halite half reproduces (halite through the ONE mechanism, Davies announced at I ≈ 3.6 mol/kg — seven times its band — so saturation is indicative: model ~3.6 vs measured 6.14 mol/kg), and the ORGANIC vertex does not form because the corpus UNIFAC is the **VLE table** (Hansen 1991), under which water/1-butanol is fully miscible at 25 °C (g_mix convex; independently confirmed by a two-liquid `propertyScanBinary` probe).  Reported, never tuned.  Built on the way, all exercised by this case: (1) the **wet organic** — the aqueous solvent as a declared organic member, its cross-liquid equality carrying the ionic a_w factor, molality on the aqueous water alone, a split⇄speciation joint fixed point inside each residual (dry path byte-identical); (2) **Gibbs-decides seeding** for the wet split — with the solvent a member, \|f\| gains TWO trivial attractors (the clone manifold and the all-organic cap corner, both at \|f\| = \|ln a_w\| with singular Jacobian) and an \|f\|-descent Newton funnels into them from any seed; a deterministic Gibbs grid + Nelder-Mead descent hands the Newton the true basin (the same division of labour the molecular LL path settled once already); (3) the speciation γ fixed point's **oscillation guard** (Davies out-of-band gives the I map a slope below −1: period-2 limit cycle 0.91 ↔ 7.8 mol/kg around a genuine fixed point at 3.65 — damped substitution, announced, floored); (4) **spectator masters are legitimate** — a fully dissociated salt's free ion is referenced by no equilibrium, and `ReactiveVLE`'s map-back verification accepted it only because an UNRELATED databank record referenced Na; the sealed frontier exposed this and the check now asks the species catalogue (`chargeOf`, promoted to the provider surface).  Gate `check_marcilla_lls` pins the verdict in BOTH directions (its stale-pin arm fires the day curation opens the gap — and the sabotage protocol caught the gate's own first draft probing a one-liquid package, the permanently-green shape).  Named remedies, sharpest first: serve **UNIQUAC** on the reactive backbone (the corpus already carries vlle01's fitted water/1-butanol pair — that alone serves the TERNARY W/B/NaCl LLS invariant, Table 4B, with in-tree data); or curate UNIFAC-LLE (Magnussen 1981); or NRTL pairs — the latter two reserved curation.  **S5b next**: the UNIQUAC backbone + the ternary invariant witness |
