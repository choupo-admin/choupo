...
$\Delta T$ -- declared as a magnitude; the engine assigns the sign from the
reaction's thermicity and announces it -- evaluates the \emph{reaction}
equilibrium at $T\pm\Delta T$ while enthalpy and the energy balance stay at
the physical $T$ -- an empirical, calibrated closeness-to-equilibrium knob
(never predicted).
...
\paragraph{The process-simulation path.}
...
sequence, recycles as tears, everything announced.  The electrodialysis
stacks sit here as the \texttt{electrodialysis} sub-class since 2026-09-20
(a top-level folder is a discipline, not a binary; the retired
\file{electrochem/} folder held these seven steady cases).
\input{tutorialsGuide-steady}

\clearpage
\section*{Category II: Transient process simulation (choupoSemiContinuous, no control loop)}\addcontentsline{toc}{section}{Category II: Transient process simulation (choupoSemiContinuous, no control loop)}\markboth{Transient process simulation}{Transient process simulation}
Unit operations connected by streams whose internal states are
NON-STATIONARY: the same $\mathrm{d}\mathbf{Y}/\mathrm{d}t = f(\mathbf{Y})$
integration in real time as the control category, with no controller
declared -- the process is watched, not driven.  A discipline of its own
(ruled 2026-09-20): the open-loop transient a student studies before any
loop is closed on it.  The binary is \code{choupoSemiContinuous}, the fifth
problem class (ruled 2026-09-20): the same driver \code{choupoCtrl} runs,
without the control layer, and a \code{controllers} block is refused there by
name; \code{choupoBatch} cases join this folder later.  Two cases today:
the cold start-up of a CSTR relaxing to its natural steady state while
OpenFOAM-style instant directories fill, and three equal mixing tanks in
series answering an impulse with the Erlang-3 residence-time density.
\input{tutorialsGuide-unsteady}

% A VALIDATION STUDY, hand-written, tables generated from the engine
% (bin/curate/gen_sdem_validation.py); written for the source paper's authors.
\input{tutorialsGuide-sdemValidation}

\clearpage
\section*{Category III: The property bench (choupoProps)}\addcontentsline{toc}{section}{Category III: The property bench (choupoProps)}\markboth{The property bench}{The property bench}
Questions asked of the thermodynamics BEFORE a simulation uses it --
CREATE, SEE, TEST, FIT, PROMOTE.
\input{tutorialsGuide-props}

\clearpage
\section*{Category IV: Batch and time-dependent (choupoBatch)}\addcontentsline{toc}{section}{Category IV: Batch and time-dependent (choupoBatch)}\markboth{Batch and time-dependent}{Batch and time-dependent}
...
\input{tutorialsGuide-batch}

\clearpage
\section*{Category V: Dynamics and control (choupoCtrl)}\addcontentsline{toc}{section}{Category V: Dynamics and control (choupoCtrl)}\markboth{Dynamics and control}{Dynamics and control}
Continuous processes in time with feedback: PID loops, setpoint schedules,
disturbances and responses.
\input{tutorialsGuide-ctrl}
