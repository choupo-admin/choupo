...
\clearpage
\section*{Category II: Transient process simulation (choupoCtrl, no control loop)}\addcontentsline{toc}{section}{Category II: Transient process simulation (choupoCtrl, no control loop)}\markboth{Transient process simulation}{Transient process simulation}
...
loop is closed on it.  The binary is \code{choupoCtrl} without a control
loop; \code{choupoBatch} cases join this folder later.  Two cases today:
