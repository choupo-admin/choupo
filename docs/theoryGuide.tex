...
Aspen and HYSYS are trademarks or registered trademarks of Aspen Technology,
Inc.
...
Part~V      & transport properties: viscosity, thermal conductivity, diffusivity, mixture rules (theory documented, code in v1.x) \\
...
when every component carries a \code{gibbsFormation} block; the report
...
component's \code{gibbsFormation} block.  Every entry in
...
derives the second half of the \code{gibbsFormation} block --- the
...
$s^{\circ}_{298}$.  That is why the \code{gibbsFormation} block carries
...
\code{fitBinaryPair} outer driver fits NRTL or Wilson parameters to a
...
used by DECHEMA and Aspen for Wilson); that extra slope is not yet wired
...
Neutral species take $\gamma=1$.  The water activity uses the dilute
(osmotic coefficient $\phi=1$) approximation
\begin{equation}
   \ln a_w \;=\; -\,M_w\sum_i m_i,
   \label{eq:spec-aw}
\end{equation}
with $M_w$ the molar mass of water in kg/mol --- consistent with the
$\phi$ definition~\eqref{eq:phi-def} at $\phi=1$.
...
exists for the case's named solvent; it then returns $K_i = H_i(T)/P$
from Eq.~\eqref{eq:henry-K} instead of the $\gamma$-$\varphi$ result.
...
\begin{tutoriallink}
In any tutorial's \file{constant/thermoPackage} you will find:
\begin{lstlisting}
...
\begin{tutoriallink}
Select the cubic EoS in any case's \file{constant/thermoPackage}:
\begin{lstlisting}
equationOfState
{
    model  SRK;            // or  PR;
...
\code{type fitBinaryPair;} in \file{system/outerDict}. Its main loop
is the half-page of code in \file{src/outerDriver/FitBinaryPair.cpp},
which calls into:
\begin{itemize}[leftmargin=*,nosep]
...
The planned syntax for the component file:
\begin{lstlisting}
viscosityLiq
{
    model         Andrade;
...
\begin{itemize}[leftmargin=*,nosep,topsep=2pt]
  ...
  \item Two-phase mixtures (multicomponent saturated states) are
        not yet supported by the \code{state} keyword --- the user
        falls back to giving $T$, $P$, $\zv$ explicitly and the
        simulator does a Rachford-Rice flash to find $\beta$.
\end{itemize}
...
\file{data/standards/membranes/} --- are the whole material model; a
...
\paragraph{Limit.}  It is built for a \emph{single-phase incompressible
liquid}.  For a gas, $\rho$ falls as $P$ drops along the pipe, so a single
$(\rho,u)$ evaluation is wrong --- compressible flow needs the
isothermal- or adiabatic-pipe integral, a documented future extension.
There is no two-phase flow, no fitting database (you furnish each $K$), and
the $K$-method itself is a lumped approximation to genuinely 3-D fitting
flows.
...
The cut model is a \emph{selectable sub-model} (the EoS/cyclone factory
pattern, ``all models are wrong, some useful''): Lapple, Leith--Licht,
Iozia--Leith, Barth, and Muschelknautz.  The last is the gold standard
because it captures the \emph{solids-loading effect} the simpler models
miss: above a limit loading, the surplus dust is mass-separated near
100\,\%, so a heavily-loaded cyclone is far more efficient than Lapple
predicts (\file{cyclone04\_high\_loading}: 98\,\% vs Lapple's 82\,\%).
...
Selecting \code{method simultaneous} on the column solves all the stage
...
unknowns than the inlet/outlet temperatures alone can fix.  Aspen's
MHeatX resolves this by demanding a fuller specification (zones, areas,
...
\file{data/standards/materials/<name>.dat} --- carbon steel, SS304,
...
\file{data/standards/materials/<name>.dat}, and $F_P$ the pressure
...
1--21 (1995). The standard derivation of the pressure- and
...
parameter-estimation driver (\code{fitBinaryPair}) inside the
