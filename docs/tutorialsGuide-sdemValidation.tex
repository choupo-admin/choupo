...
fitted); ideal dilute virtual solutions; constant permeances; and, in
these runs, a concentration-polarisation film that is computed per ion
\emph{without} a field (the default, which the engine announces every
run).  A coupled film --- the interface model of Geraldes and Afonso
(2007), one potential gradient at the wall fixed by electroneutrality,
declared by \code{polarisation \{ ionCoupling electroneutral; \}} ---
exists in the engine and is deliberately \emph{not} used here: all runs
below use a film coefficient of 1~m/s, so the wall equals the bulk to
better than $0.01\,\%$ and the rejections are intrinsic --- which is what
the paper's CP-corrected rejections are, and a film that does not matter
has no field to carry.  Numerically, each station solves a Newton problem in the
...
Figure~\ref{fig:sdem-validation} draws the whole comparison at once; the
tables that follow carry the same points with their numbers.  Read it
series by series: where the solid line sits on the dashed one, Choupo in
the trace limit reproduces the authors' line; where the dotted line leaves
both, the paper's own trace concentration is no longer a trace for that ion
(the section on that point below); and where the markers leave every line,
the residual is the authors' fit against their own measurement, which no
implementation of the same model can close.

\input{tutorialsGuide-sdem-figure}
...
\input{tutorialsGuide-sdem-ambipolar}
...
\input{tutorialsGuide-sdem-NaCl}
...
\input{tutorialsGuide-sdem-MgCl2}
...
\input{tutorialsGuide-sdem-Na2SO4}
...
\input{tutorialsGuide-sdem-MgSO4}
...
\input{tutorialsGuide-sdem-notatrace}
...
\input{tutorialsGuide-sdem-sensitivity}
...
does not exercise the coupled polarisation film (the runs are intrinsic;
that film is the subject of tutorial \code{membrane14} and of the gate
\code{check\_polarisation\_coupling}), concentration-dependent
