...
## 1. Why an index, when 73 records already exist
...
- Of 73 design records, **36 state a rejected alternative and 37 do not** —
...
| [`one-tab-one-thing.md`](../design/one-tab-one-thing.md) | ADR | yes | **DECIDED 2026-08-17** (Vítor) — the hub tab stays open; a case, Explore and the EduTools each open a NEW tab, so the mode/view distinction becomes STRUCTURAL instead of a rendering rule inside one strip, and a case tab's menu carries only its own views.  Makes the credo's "the address bar is a shareable bookmark" true and testable.  Hard consequence stated up front: separate tabs do not share a store, and ten of twelve tools read the open run, so a tool tab must be GIVEN the case in its address and solve it itself — which also makes the comparison shareable.  REJECTED: cross-tab state sync (one tab's answer depending invisibly on another tab's lifetime), a tab per VIEW, and any standalone-vs-in-app fork of a tool.  SUPERSEDES the navigation half of modes-and-views; fires that record's own third falsifier exactly as predicted |
...
**37 of 73 records state no rejected alternative.**  For a FORUM or a STUDY
