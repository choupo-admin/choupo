...
| **Ice is a PHASE of the solvent, not a special case** (`SolidPhase::fEffective`; freezing-point depression falls out of K = 1) | [`../design/ice-as-a-solid-phase-of-the-solvent.md`](../design/ice-as-a-solid-phase-of-the-solvent.md) | five named refusals, each carrying its remedy | `check_ice_freezing` (**4 sabotages**; the crossing, the slope's sign AND magnitude, and the purity claim no balance check could catch) — **NO CASE FIRES IT**: the witness tutorial waits for a published anchor | **witnessed 2026-08-08 — `fpd01_nacl_freezing` consumes SolidPhase through the `freezingPoint` op (AAD 0.05 % at the interim anchor); the FLASH solid path stays absent pending the C2 spike** |
