...
| Docs for LLMs | `docs/ai/` + AGENTS.md + README + CLAUDE.md + all 7 guides (incl. the split tutorialsGuide-*.tex) teach v2 only; the retired-name gate §5 enforces it |
...
3. **`constant/electrolyte/` transitional adapters (2608).**  "v1 is
   dead" is a statement about the THERMO GRAMMAR only.  The multi-ion
   speciation front-end still consumes case-local
   `constant/electrolyte/{speciation,ions}.dat` sidecars — 3 live files in
   2 tutorials (`pitzer_seawater_verify`, `tartaricAcid_acidulation`) —
   through adapters in `SaltFromCatalogue.H`, `SpeciationSolver`,
   `Speciate`/`Exchange`, `IonExchanger` and related unit ops.  The guides
   name them transitional adapters; folding them into the sealed
   `species/`+`chemistry/` closure is 2608 work.
4. **Landing page mobile.**  At 390×844 the landing clips horizontally
   (Codex smoke, confirmed broken: lockup, copy, CTAs and cards all clip).
   Codex's 2026-07-19 landing addendum (Vítor-requested) also asks for
   capability-aware CTAs and the help-request demotion.
5. **`docs/ai/consistency.md`, `extending.md`, `gui-credo.md`** were not
   part of the v2 scrub sweep (no retired-token hits — the extended
   retired-name gate now watches them — but nobody re-read them end-to-end
   this wave).
6. **Batch/ctrl have no default element report** — deliberate (they carry
   their own ledgers), but the asymmetry should be stated in the manuals
   in a future pass.

   *(Closed since first writing: the help-index/guide coverage debt and
   the stale userGuide reports table — `1b930541e`; the tutorialsGuide
   legacy pass — the commit carrying this update.)*
...
cd gui && npx tsc --noEmit && npx vitest run   # 1811 tests
...
EXPECTED-FAIL 4**; vitest **1811/1811**; release inventory generated by
...
- COSMO-SAC sets: VT2005 (US-gov public domain via NIST bundle) on 77
  ...
  *(Correction 2026-09-06, the snapshot text kept as written: the VT2005 set
  is NOT public domain — NIST's own README puts the bundled profile folders
  under an academic, non-commercial permission, and the public tree ships
  no value; see `docs/design/the-licence-of-a-sigma-profile.md`.)*
...
The wave sits on `cleanup/binarypairs-parameters-tidy` ahead of the
remote (154 at the time this list was captured; later closing commits —
`1b930541e` guides v2 + balances chapter, `8ad8dc9c7` docs/ai last echoes,
and the documentary-truth pass carrying this update — follow the same
conventions).  Regenerate the exact list with `git log --oneline @{u}..HEAD`.
