...
\addlinespace[2pt]
\multicolumn{2}{@{}l}{\itshape Caloric anchors --- \code{gibbsFormation}} \\
...
\begin{quote}
\texttt{constant/propertyPackage} \emph{declares} $\;\rightarrow\;$ the builder
...
model per PHASE & \texttt{propertyMethods \{ liquid ...; vapour ...; \}} & ONE Gibbs surface per phase --- the liquid slot selects the VLE world\\
...
\item \textbf{Proposed, by name} --- \texttt{data/proposed/components/} is the
      larger review queue.  Records are runtime-usable, but every use is
      announced as \texttt{[proposed]}; a proposed record never shadows a
      standard one.  Integrity and provenance gates make a proposal
      inspectable, not automatically authoritative.
...
proposed}.  This is an evidence ladder, not merely a search path: moving a file
up the ladder requires review and validation, not a rename.

\subsection{The present ChemSep review queue}

The ChemSep 8.3 importer is deliberately reproducible and reviewable.  After
CAS-based de-duplication it contributes 279 new component records to a proposed
catalogue of 625 records in total.  All 625 pass the structural/provenance
audit.  Of the 279 new ChemSep records, 236 pass the internal
$P^{\mathrm{sat}}(T_b)$ check within 5\%; 43 are flagged for review.  This is a
closure check on the imported constants and predictive correlation, not an
independent comparison with laboratory data.

The same import stages 748 NRTL, UNIQUAC and Wilson binary-pair records.  All
748 pass identity, direction, licence, source-digest and exact unit-conversion
...
\texttt{[proposed] UNVERIFIED} when one is selected.  Pair resolution is
\textbf{case-local/inline $>$ standard $>$ proposed $>$ announced ideal
default}; proposed data can therefore extend a case without silently changing
an approved standard.

The source boundary is intentional.  The unmodified third-party database and
its Artistic-2.0 licence are kept under \texttt{thirdParty/chemsep/}; only
normalised, attributed review records are generated under
\texttt{data/proposed/}.  Literature values retain literature provenance;
...
\code{propertyPackage} grammar: \emph{models per phase; conventions per
...
\end{array}
\]
Each row is one \code{propertyMethods.liquid} family
(\code{activity.*}, \code{solution.henryDilute}, \code{eos.*},
\code{electrolyte.*}); the run log announces which world is live.  All four
...
data/standards/
  ...
  species/aqueous/<sp>.dat         model species (Na, Cl, CO2aq): charge +
  ...
  propertyMethods/.../<name>.dat   a model declaration (+ its reference basis)
  (the package is the manifest INLINE in the case's constant/propertyDict --
   THE CENTRE; there is no shared propertyPackages/ catalogue)
\end{lstlisting}
...
\paragraph{Selecting a package.}  A case does not list loose models and
parameters.  It \emph{selects} a \code{propertyPackage} --- one record that
names the components, the methods, and the parameter banks the run needs:

\begin{lstlisting}
recordType propertyPackage;   name aqueousNaCl_pitzer;
components        ( water NaCl );   // NaCl's dissociatesTo names the ions
chemistry         { salts ( halite ); }   // the builder reads salts; the
                                           // solubility anchor comes from the
                                           // salt record itself
propertyMethods   { liquid electrolyte.pitzer; vapour builtin.idealGas; }
parameters        { pitzerPairs { Na-Cl ".../pitzer/pairs/Na-Cl.dat"; } }
% NOTE: pitzerPairs is today a CITATION of where the pair lives; the loader
% resolves the pair by (cation,anion) name. kijPairs/henryPairs/binaryPairs
% are loaded from their declared paths and refuse when missing.
\end{lstlisting}

The flow is one sentence: \textbf{\code{propertyPackage} declares,
\code{ThermoPackageBuilder} assembles, \code{ThermoPackage} computes.}  The
package is the recipe; the builder loads what it names and mounts the runtime
object the unit operations consume.  An ordinary molecular case is the same
record degenerate --- no dissociation, no chemistry, an NRTL or EoS model,
optional binary pairs; the old flat \code{thermoPackage} dict you already
write \emph{is} that degenerate package.
...
\emph{loads} existing case, standard or proposed records; it never invents a
...
clear message.  So: estimate, look, promote --- and inspect every proposed or
...
\file{constant/binaryPairs/<model>/<pair>.fit-<date>.dat}.  Promotion is a
