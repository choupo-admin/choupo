...
\texttt{tutorials/steady/reactors} cases; for economics-driven X,
...
\textbf{Why the top and the bottom of a column want different diameters.}
The flooding diameter follows the \textbf{vapour VOLUMETRIC flow}, not the
molar one, and the two sections differ in every term that turns one into the
other: temperature, pressure, vapour molar mass and usually the vapour molar
flow itself. Below the feed the liquid traffic is larger, which raises the flow
parameter F\_LV = (L/V)\(\surd\)(\(\rho\)\_V/\(\rho\)\_L) and therefore
LOWERS Fair's capacity parameter C\_sb --- so a stripping tray floods at a
lower velocity than a rectifying one. Whether that makes the bottom the wider
section is not a rule: the vapour volumetric flow may fall faster than the
capacity does, and then the top governs. It is decided by whichever tray loses
the argument, tray by tray, which is exactly what
\texttt{column09\_tray\_hydraulics} prints. It reports \textbf{1.21 m} for
its rectifying section against \textbf{1.32 m} for its stripping section, on
the same trays and the same spacing.

\textbf{Swaging is an ECONOMIC decision, not a thermodynamic one.} Having two
diameters, you may build a \textbf{swaged} tower --- a narrow section, a
transition cone and a wide section --- or one \textbf{straight} shell at the
larger diameter. The straight shell buys plate you do not need; the swaged one
buys a transition, its fabrication, its extra weld inspection and a second set
of tray sizes to hold in stores. The usual rule of thumb: \textbf{swage only
when the two sections differ by roughly 15--20\% in diameter}, and build
straight below that, because under about 15\% the transition costs more than
the plate it saves. Judge it on DIAMETER, not on area --- 20\% in diameter is
already 44\% in cross-section. The 8\% gap in the case above is comfortably
inside the band, so that tower is built straight at 1.32 m, and Choupo says so
without deciding for you: it publishes both section diameters and their
difference and prices no transition, because it has no correlation for one.
...
\textbf{What Choupo can and cannot check for you, stated before you rely on
it.} An earlier version of this paragraph invited you to ``step a disturbance
in \texttt{ctrl01} and watch whether your loop rejects it or fights another''.
That invitation was false three ways, and it is corrected here rather than
quietly deleted, because a manual that has promised something owes the reader
the correction: \texttt{ctrl01} carries no disturbance at all (it is a
setpoint-tracking case --- the disturbance cases are \texttt{ctrl02},
\texttt{ctrl05}, \texttt{ctrl09}); \emph{no} case in the corpus carries more
than one PID loop, so ``fights another'' cannot be watched anywhere; and this
section is about \emph{column} pairing, while the dynamic tier has no column
in it. A pairing for a column cannot be tested dynamically in Choupo today.

What you \emph{can} do, and it answers the sharpest question in this section
--- \emph{which tray carries the temperature measurement} --- is read it off
the rigorous column you already ran. The steady profile gives the slope
criterion directly, and re-solving with a manipulated variable nudged gives
the sensitivity criterion. The EduTools \emph{Column control} tool does
exactly that (\S\ref{sec:column-control}) and reports something a single
answer would hide: on a 15-stage column the slope criterion picks stage~10,
the reflux handle picks stage~11, and the distillate handle picks stage~5 ---
a different section of the column, with a response fifteen times stronger. The
same run shows the reflux response \emph{changing sign} between stages~6
and~7, which no slope of the profile can reveal.

Choupo computes no relative-gain array, at any frequency: there is no
steady-state gain matrix in the engine. The pairing heuristics below argue in
terms of relative gains, and that argument is the literature's, not a number
this simulator produced.
