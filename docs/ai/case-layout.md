...
├── converged/               GENERATED: the solved stream state, one file per
│                              stream, owned by its producing sector (gitignored)
...
├── internalStates/          GENERATED: WHAT HAPPENS INSIDE EACH UNIT,
│   └── <SECTOR>/<unit>/<kind>   one dictionary per unit that publishes a
│                              profile -- `stageProfile`, `axialProfile`,
│                              `sizeDistribution`, `swingTable` (or `profile`
│                              for an axis with no declared kind).  A PROJECTION
│                              of the same record as `unitOperations/<unit>/
│                              profile.csv` above; rewritten whole every run,
│                              never edited (gitignored).  A temperature-swept
│                              construction (van Heerden, Merkel) is an
│                              analysis, not equipment state, and gets no file.
...
internalStates  what happens inside it   (one file per unit that publishes a profile)  [run output]
...
`0/`, `converged/`, `design/` and `internalStates/` all carry
...
`internalStates/` is a projection of the profiles the units publish — its
boundary (a field over a coordinate of the equipment or its inventory, never
a construction over a swept parameter) is in
