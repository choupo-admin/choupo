...
## Closing state, 2026-08-22 end of day

The full regression suite, on a clean tree at the audit's final commit:

    PASS 512 / FAIL 0 / KNOWN-BROKEN 0 / EXPECTED-FAIL 5      (exit 0)

Two honest SKIPs (both name `gui/node_modules` as absent and are not counted
as passes).  The morning's state was a suite RED on every clean clone with
eight gates that could not fail behind the green.

Landed during the day, each slice pushed as it closed: the eight
permanently-green gates rewritten with arms that fail (sabotage-verified);
the DirectMin fabricated-convergence fix with its golden re-recorded (physics
byte-identical); the new `check_tb_antoine` gate with its 13 pins moved into
`debt_registry.py` after `check_debt_registry` refused the audit's own first
version -- the machinery catching its auditor; the doc-reference and
guide-path gates taught the two run-output spellings that made them red on
fresh clones; the level-1 prose brought back to what the layering gate
measures; and the derived views (code map, tutorials guide) regenerated by
their own generators.

Still open, deliberately: the 13 pinned Tb/Antoine records await primary
literature; the pruning list (17 registered-but-unexercised models, 23
smoke-only cases) awaits the architect's witness-or-discard ruling; and the
96 unread gates from the coverage statement remain unread.
