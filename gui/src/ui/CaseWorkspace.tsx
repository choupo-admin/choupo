...
export function CaseWorkspace() {
  ...
  //  `converged/internalStates/<SECTOR>/<unit>` (one file per unit) is part
  //  of the same view as the streams and arrives on the same channel.
  const raw = useMemo(
    ...
    return (
      <Stack key={node.prefix} gap={0} mb={depth === 0 ? 4 : 0}>
        ...
        >
          ...
          {/*  THE UNIT INTERIORS of a state view: the stream files beside this
               root are the boundary, and under it sits what each unit holds
               between them, ONE file per unit -- the OpenFOAM reading of a
               time directory, where one field file carries its boundary
               conditions and its internal field.  */}
          {kind === "interior" && (
            <Tooltip
              label="What each unit holds inside it at this state -- one file per unit, a block per kind (stage profile, axial profile, size distribution). Copy a file into 0/internalStates/ at the same address to declare the interior the next run starts from."
              withArrow
            >
        ...
        {!isCollapsed && (
          <>
            {/*  A STATE VIEW READS BOUNDARY FIRST.  Its stream FILES are the
                 boundary of the snapshot and `internalStates/` is what sits
                 between them, so that root is drawn after the leaves --
                 every other kind of child still comes first.  */}
            {sortedChildren(node)
              .filter((k) => nodeKind(k) !== "interior")
