    ...
    <>
        ...
        <TopMenu label="Help" px={shape.padX}>
          ...
          {/* EVERY manual docs/Makefile publishes, in ITS order.
              ...
              `DOCS` line, so adding a guide there puts it here.  The
              Props Workflow Guide keeps its own label: `propsGuide` is
              the props-mode WORKFLOW (estimate -> fit -> consistency
              test), not the property THEORY, and a menu entry reading
              "Properties Guide" invites exactly that confusion (forum
              2026-06-15). */}
          {GUIDES.map((g) => (
            <Menu.Item
              key={g.name}
