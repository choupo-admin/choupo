  ...
  const rowItems = (shape: { collapsed: boolean; padX: number }) => (
    <>
        ...
        <TopMenu label="Help" px={shape.padX}>
          ...
          <Menu.Divider />
          {/* EVERY manual docs/Makefile publishes, in ITS order.
              This block used to name four by hand -- and its own comment
              said "The four glass-box manuals" -- while DOCS had grown to
              eight.  So the Design Guide (2400 lines of heuristics and
              rules of thumb) was compiled, copied into public/docs and
              published to the site, with no way to reach it from inside
              the app: reported as DELETED, which is what an unreachable
              document looks like from outside.

              GUIDES is generated by scripts/copyDocs.mjs from the same
              `DOCS` line, so adding a guide there puts it here, and each
              label is the guide's OWN `\docTitle` rather than a table
              anybody maintains.  That also settles two things by
              construction: propsGuide reads "Props Workflow Guide"
              because its .tex says so (it is the props-mode WORKFLOW,
              not the property THEORY -- forum 2026-06-15), and
              designGuide reads "Process Design Heuristics", which is
              what it IS and what the old hand-written "Design Guide"
              was hiding. */}
          {GUIDES.map((g) => (
            <Menu.Item
              key={g.name}
              onClick={() => openGuide(guideUrl(g.name))}
            >
              {`${g.label} (PDF)…`}
            </Menu.Item>
          ))}
