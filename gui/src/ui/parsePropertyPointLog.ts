...
export function parsePropertyPointLog(log: string): PointResult[] {
  // Split log into per-operation blocks at each ">>>  Operation [N]:" marker.
  const blocks = log.split(/(?=>>>\s+Operation\s+\[\d+\]:)/);
  const out: PointResult[] = [];
  for (const blk of blocks) {
    const hdr = blk.match(/>>>\s+Operation\s+\[\d+\]:\s+(\S+)\s+\(type\s*=\s*(\w+)\)/);
    if (!hdr) continue;
    const [, name, type] = hdr;
    if (type !== "propertyPoint") continue;
    ...
    const T_K = num(new RegExp(`T\\s*=\\s*${NUMBER}\\s*K`));
    const P_bar = num(new RegExp(`P\\s*=\\s*${NUMBER}\\s*bar`));
    if (T_K === undefined || P_bar === undefined) continue;

    out.push({
      name: name!,
      T_K,
      P_bar,
      composition: extractComposition(blk),
      H_ig:   num(new RegExp(`H_ig\\s*=\\s*${NUMBER}`)),
      S_ig:   num(new RegExp(`S_ig\\s*=\\s*${NUMBER}`)),
      Cp_ig:  num(new RegExp(`Cp_ig\\s*=\\s*${NUMBER}`)),
      gamma:  num(new RegExp(`gamma=Cp/Cv\\s*=\\s*${NUMBER}`)),
      Z:      num(new RegExp(`\\bZ\\s*=\\s*${NUMBER}`)),
      S_real: num(new RegExp(`S_real\\s*=\\s*${NUMBER}`)),
    });
  }
  return out;
}

/** Pull a compact composition string from the "Composition (mole frac):"
 *  sub-block.  Pure components return just the name; mixtures concatenate
 *  "name:fraction" pairs with 2-decimal fractions. */
function extractComposition(blk: string): string {
  const start = blk.search(/Composition\s*\(mole\s*frac\):/);
  if (start < 0) return "";
  const lines = blk.slice(start).split("\n").slice(1);
  const parts: string[] = [];
  for (const line of lines) {
    const m = line.match(/^\s+([A-Za-z][A-Za-z0-9_]*)\s+([\d.]+)\s*$/);
    if (m) {
      const x = Number(m[2]);
      parts.push(x === 1 ? m[1]! : `${m[1]}:${x.toFixed(2)}`);
    } else if (parts.length > 0) {
      // First non-matching line after at least one match ends the block.
      break;
    }
  }
  return parts.join(" ");
