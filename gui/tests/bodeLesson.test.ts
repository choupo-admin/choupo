...
describe("every file:line this page cites into the engine resolves", () => {
  ...
  const ANCHORS: [string, number, string][] = [
    ...
    ["src/dynamicDriver/DynamicDriver.cpp", 1595, "std::hypot(abc[1], abc[2])"],
    ["src/dynamicDriver/DynamicDriver.cpp", 1597, "std::atan2(abc[2], abc[1])"],
    ["src/dynamicDriver/DynamicDriver.cpp", 1609, "out_phase_rad"],
    ["src/dynamicDriver/DynamicDriver.cpp", 1614, "fit_residual_rel"],
    // the drive
