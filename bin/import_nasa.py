...
/*--------------------------------*- Choupo -*-----------------------*\\
  ...
  (Burcat 2005 / JANAF).  Critical properties from Reid-Prausnitz-
...
def parse_chemkin_thermo(path: Path) -> dict[str, dict]:
    ...
    while i + 3 < len(raw):
        ...
        # Elements: pairs of (2-char symbol, 3-char count) at cols 25-44
        elems = {}
        for j in range(5):
        ...
        if len(coefs) >= 14:
            out[name] = dict(
                ...
                formula=name,
                cas="?",
                mw=0.0,            # caller must supply if missing
                elements=elems,
                low=coefs[7:14],
                high=coefs[0:7],
            )
