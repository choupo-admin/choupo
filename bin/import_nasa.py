...
#  A DECLARED WINDOW IS A CLAIM ABOUT THIS FIT, and both .dat templates
#  used to TYPE one -- the nonvolatile template said `Trange (250 3000)`
#  beside coefficients regressed over [250, 1500].  A cubic through Cp is
#  superb inside its range and diverges fast outside it; measured against
#  the NASA-7 truth, S2 is +0.6 % at 1500 K, +11.7 % at 2000 K and +107 %
#  at 3000 K, and S8 reaches +196 %.  The engine HONOURS a declared window,
#  so 1500 K of invented validity bought silence exactly where the fit
#  stops working -- and one shipped case (gibbs06_h2_flame_radicals, 2386 K)
#  sits in it.  The templates now interpolate the window from this
#  function's own arguments: one home, so the two cannot disagree.
#
#  NOT FIXED BY WIDENING THE FIT.  Refitting over [250, 3000] would move
#  every value BELOW 1500 K too -- a wider fit is worse in the range that
#  is actually used -- so it is a curation act that moves goldens, not a
#  relabelling.  The real answer is a NASA-7 heat-capacity model in the
#  engine beside the polynomial, and that is reserved.
...
{{
    ...
    //  the window IS the fit's range -- see fit_cp_poly3
    Trange        ({cp_tmin:g}  {cp_tmax:g});
}}
"""
...
def make_dat(name: str, entry: dict, outdir: Path) -> Path:
    ...
    CP_TMIN, CP_TMAX = 250.0, 1500.0
    cp_coefs = fit_cp_poly3(low, high, T_min=CP_TMIN, T_max=CP_TMAX)
    ...
    payload = dict(
        ...
        cp_tmin=CP_TMIN, cp_tmax=CP_TMAX,
    )
    if crit:
        payload.update(crit)
...
TM4513_SOURCE = ("NASA TM-4513, McBride, Gordon & Reno 1993 -- a US "
                 "government work, PUBLIC DOMAIN; read from cantera's "
                 "bundled nasa_gas.yaml")


def parse_nasa_tm4513(names: list[str]) -> dict[str, dict]:
    """Build importer entries from NASA TM-4513, the CLEAN source.

       WHY THIS PATH EXISTS.  `--thermo` was written for a CHEMKIN file, and
       in practice the file people reach for is BURCAT.THR -- which is
       NonCommercial and must not be the origin of a shipped value
       (debt_registry.NC_COMPILATION, and the design record
       a-noncommercial-compilation-in-the-public-tree.md).  TM-4513 is the
       remedy the tree already names, so the tool that performs imports
       should be able to reach it directly rather than leaving the
       encumbered path as the only convenient one.

       Needs `cantera` for the bundled table.  REFUSES rather than falling
       back: silently importing from somewhere else would be the exact
    ...
    try:
        ...
        raise SystemExit(
            "import_nasa --tm4513: cantera is not installed (`pip install "
            "cantera`), and there is no fallback -- falling back to another "
            "table would put a different source behind the same banner.")
    pool = {s.name: s for s in ct.Species.list_from_file("nasa_gas.yaml")}
    out: dict[str, dict] = {}
    for n in names:
        s = pool.get(n)
        if s is None:
            print(f"  ?? '{n}' is not in NASA TM-4513; skipping",
                  file=sys.stderr)
            continue
        c = list(s.thermo.coeffs)          # [Tmid, high(7), low(7)]
        comp = {k: int(v) for k, v in s.composition.items()}
        formula = "".join(f"{e}{comp[e] if comp[e] > 1 else ''}"
                          for e in sorted(comp))
        mw = sum(ATOMIC_MASS[e] * k for e, k in comp.items())
        out[n] = {"low": c[8:15], "high": c[1:8], "formula": formula,
                  #  CAS is NOT in the table, and inventing one would be a
                  #  fabricated identifier in an identity field.
                  "cas": "n/a", "mw": mw, "source": TM4513_SOURCE,
                  "comment": f"{n} (from NASA TM-4513)"}
    return out
...
def main() -> int:
    ...
    p.add_argument("--tm4513", nargs="+", metavar="SPECIES",
                   help="Import these species from NASA TM-4513 (public "
                        "domain, via cantera's nasa_gas.yaml) instead of a "
                        "CHEMKIN file")
    args = p.parse_args()
    ...
    if args.tm4513:
        db.update(parse_nasa_tm4513(args.tm4513))
    if args.thermo:
    ...
    targets = (args.species or args.tm4513
               or list(NASA7_INLINE.keys()))
    for name in targets:
        if name not in db:
