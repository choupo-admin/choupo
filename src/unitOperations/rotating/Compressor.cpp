...
int Compressor::solve(const DictPtr& dict,
                      ...
                      int verbosity)
{
    ...
    if (eta <= 0.0 || eta > 1.0)
        throw std::runtime_error(
            "Compressor: eta must satisfy 0 < eta <= 1 (got "
            + std::to_string(eta) + ")");
    if (W_shaft <= 0.0)
        throw std::runtime_error(
            "Compressor: W_shaft must be positive (use a turbine block for "
            "expansion).  Got " + std::to_string(W_shaft) + " W");
    ...
    const scalar w_real_per_mol = W_shaft / F_mol_s;    // J/mol
    // Compressor convention: eta = w_isen / w_real  =>  w_isen = eta*w_real.
    const scalar w_isen_per_mol = eta * w_real_per_mol;

    // ---- Discharge solve (shared core) ----------------------------------
    auto disc = rotating::solveDischarge(thermo, T_in, P_in, y, h_in, s_in,
        w_real_per_mol, w_isen_per_mol,
        /*expansion=*/false, verbosity);
    if (!disc.converged)
        throw std::runtime_error(
            "Compressor: discharge solve did not converge");
    ...
    if (verbosity >= 2)
    {
        std::cout << "\n============================  Compressor Result  ====================\n"
                  ...
                  << W_real_kW << "  kW   |   eta = " << std::setprecision(4) << eta << "\n"
                  ...
                  << "  W_isen:          " << std::fixed << std::setprecision(3)
                  << W_isen_kW << "  kW   (eta * W_shaft)\n"
    ...
    out.P    = P_out;
    out.z    = y;
    out.vf   = 1.0;     // gas-phase by construction
    produced_.push_back(out);

    // ---- KPIs -----------------------------------------------------------
    kpis_.clear();
    kpis_["W_shaft"]    = W_shaft;
    kpis_["W_shaft_kW"] = W_real_kW;
    kpis_["W_isen"]     = W_shaft * eta;
    kpis_["W_isen_kW"]  = W_isen_kW;
    kpis_["eta_isen"]   = eta;
    kpis_["P_in"]       = P_in;
    kpis_["P_out"]      = P_out;
    kpis_["ratio"]      = ratio;
