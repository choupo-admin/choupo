                         ...
                         int verbosity)
{
    ...
    if (model == "geometry")
    {
        ...
        const int    nTubes  = static_cast<int>(g->lookupScalar("nTubes"));
        const int    passes  = static_cast<int>(g->lookupScalarOrDefault("passes", 1.0));
        const scalar shellID = g->lookupScalar("shellID", Dims::length);
        ...
        if (tubeID <= 0 || tubeOD <= tubeID || tubeL <= 0 || nTubes <= 0
            || shellID <= 0 || baffle <= 0 || pitch <= tubeOD)
        ...
        // Equivalent diameter (square pitch): D_e = 4(p^2 - pi d_o^2/4)/(pi d_o).
        const scalar D_e = 4.0 * (pitch * pitch - M_PI * tubeOD * tubeOD / 4.0)
                         / (M_PI * tubeOD);
        ...
        geomKpis["controllingResistance"] =
