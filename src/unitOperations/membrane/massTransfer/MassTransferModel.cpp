...
std::map<std::string, MassTransferModel::Factory>& MassTransferModel::registry()
{
    static std::map<std::string, Factory> r;
    return r;
}

void MassTransferModel::registerType(const std::string& name, Factory f)
{
    registry()[name] = std::move(f);
}

std::unique_ptr<MassTransferModel> MassTransferModel::New(const std::string& name)
{
...
std::vector<std::string> MassTransferModel::availableTypes()
{
