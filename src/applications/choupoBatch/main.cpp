...
try
{
    ...
    static const char* USAGE =
        "Usage: choupoBatch [options] [case-directory]\n"
        "\n"
        "  Batch / time-dependent solver, driven by the `recipe` block of the\n"
        "  case's flowsheetDict.  With no case directory it runs the current\n"
        "  directory.\n"
        "\n"
    ...
    //  steady flowsheet reads system/solverDict, so its presence in a batch
    ...
    if (verbosity >= 1 && fs::exists("system/solverDict"))
        std::cerr << "[dict] system/solverDict is present but choupoBatch"
                     " does not read it -- every value in it had NO effect"
                     " on this run.  Batch numerics live in each unit's"
                     " `solver {}` block (integrator, rtol) and in"
                     " controlDict `timeStepping` /"
                     " `timeSteppingControl {}`.\n";
