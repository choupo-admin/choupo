...
-------------------------------------------------------------------------------
File
    src/dynamicDriver/DynamicDriver.cpp

Description
    THE ONE HOME OF THE DYNAMIC DRIVER -- the time-integrated flowsheet of
    dynamic unit operations connected by streams.  Two applications run it:

        choupoCtrl            the driver + the CONTROL layer (controllers
                              writing manipulated variables at each sample)
        choupoSemiContinuous  the driver alone -- continuous transients,
                              fed-batch, feed & bleed, no control loop

    Extracted VERBATIM from src/applications/choupoCtrl/main.cpp on
    2026-09-20 (Vitor's ruling: "Como e que um aluno sabe que choupoCtrl
    simula um reactor feed and bleed?!  Cria a classe semiContinuous!").
    The extraction moved no digit: every ctrl and unsteady case's stdout,
    trajectory.csv, balanceTrajectory.csv/.meta, reports and instant
    directories compared byte-identical before and after.  What differs
    between the two applications is carried by DynamicDriverConfig -- the
    binary's name (which prefixes every refusal), its banner suffix, its
    usage text, the tag SolutionWriter stamps on each instant -- and nothing
    else; the driver holds no `if (binary == ...)`.

    Record: docs/design/a-class-a-student-can-name.md

    The time loop:

        for t in [startTime, endTime] step deltaT:
            for each controller:  controller.update(t, dt)   (none: no loop)
    ...
    Case dict layout:
        ...
        │   └── flowsheetDict    units list (+ controllers list under choupoCtrl)
        └── constant/
...
try
{
    printBanner(cfg.bannerSuffix.c_str());
    ...
    //  here: a driver without a control loop must not register one.

    const char* USAGE = cfg.usage;
    ...
    if (unitList.empty())
        throw std::runtime_error(cfg.binaryName + ": flowsheetDict has no units");
            ...
            if (!routes.empty() && cDict->found("actuator"))
            {
                ...
                for (const auto& r : routes)
                    if (unitNames[r.to] == tgt && act->found("inletField"))
                        throw std::runtime_error(cfg.binaryName + " routing:"
    ...
    if (writeOutputs && !csv)
        throw std::runtime_error(cfg.binaryName + ": cannot open trajectory.csv");
    csv << "t";
    for (const auto& u : units)
