                      ...
                      const fs::path&       file)
{
    ...
    //  -- and the reader refuses the ambiguous combination (a top-level block
    //  beside a phases{} that names an aqueous phase).
    ...
    if (hasSpec && !splitLiquids) writeSpeciation("");

    // PHASE DECOMPOSITION -- written when a solid phase rides along, or when a
    // second liquid was declared and is present.  It decomposes the overall
    // material and sums back to it (validated on read).
    bool anySolid = false;
    ...
    if (anySolid || splitLiquids)
    {
            ...
            for (std::size_t i = 0; i < thermo.n(); ++i)
                if (significant(q(i)))
                    out << "            " << thermo.comp(i).name() << "    "
                        << q(i) * 3600.0 << " kmol/h;\n";
            out << "        }\n";
            if (nestSpeciation) writeSpeciation("        ");
            out << "    }\n";
        };
        out << "\nphases\n{\n";
        if (splitLiquids)
        {
            ...
            //  at whatever precision this file is written in.  Storing both
            //  would put one quantity in two homes and let them disagree in
            //  the last digits, which is the closure the reader checks.
            const auto& org = *s.organicLiquid;
            auto orgOf = [&](std::size_t i)
            { return i < org.size() ? org[i] : 0.0; };
            block("aqueous",
                  [&](std::size_t i)
                  { return s.F * (i < s.z.size() ? s.z[i] : 0.0) - orgOf(i); },
                  hasSpec);
            block("organic", orgOf);
        }
        else
        {
            block(fluidPhase,
                  [&](std::size_t i){ return s.F * (i < s.z.size() ? s.z[i] : 0.0); });
        }
        if (anySolid)
                              ...
                              const ThermoPackage&  thermo)
{
    ...
    //  the speciation block that rides inside it.  These are what the
    //  speciation is checked against: the ions live in the aqueous phase and
    //  nowhere else, so checking the block against the OVERALL material is
    //  right only while no other phase holds a speciating component.
    std::vector<scalar>       aqueous;         // empty = no aqueous phase named
    std::shared_ptr<const Dictionary> nestedSpec;
    std::string               specPhase;       // which phase carried it
    if (d->found("phases"))
    {
        ...
        //  A speciation may not sit at the top level while a phases block
        //  names the aqueous phase: that is the ambiguity this whole change
        //  exists to remove -- the block would be checked against the overall
        //  material when its own phase is right there, named.
    ...
    if (d->found("speciation") || nestedSpec)
    {
        ...
        //  amounts.  Top-level: the whole stream -- which is correct exactly
        //  when there is one liquid and no other phase holds a speciating
        //  component, and that is why the nested form exists at all.
        const std::vector<scalar>& basisMaterial =
            nestedSpec ? aqueous : overall;
        if (!cfg)
